...
n_sims <- 1e5
n_mms <- 4
mms <- sample(names(probabilities), size = n_sims * n_mms, replace = TRUE, prob = unlist(probabilities))
mms <- matrix(mms, nrow = n_sims) # produce a matrix containing the result of every simulation
...
# insert R code here
p_cabin <- 0.18
p_lake_given_cabin <- 0.35
p_cabin * p_lake_given_cabin
```
...
plot(p, p_match, xlab = "p", ylab = "Probability of match", col = "green")
```
